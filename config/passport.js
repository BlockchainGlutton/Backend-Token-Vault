...
const localSignupStrategy = (req, address, password, done) => {
  if (address.length === 42) {
    address = address.toLowerCase(); // Use lower-case e-mails to avoid case-sensitive e-mail matching
  }
  // asynchronous
  process.nextTick(() => {
    ...
    if (!req.user) {
          ...
          bcrypt.genSalt(10, (errC, salt) => {
            bcrypt.hash(password, salt, (errCrypt, res) => {
              const newUser = {
                address,
                password: res,
                aliasname: generateRandomString(7),
              };
      ...
      User.findOne({ address }, (err, rUser) => {
        if (err) {
          return done(err);
        }
        if (rUser) {
          ...
          bcrypt.genSalt(10, (errC, salt) => {
            bcrypt.hash(password, salt, (errCrypt, res) => {
              ...
              newUser.aliasname = generateRandomString(7);
...
const localSigninStrategy = (req, address, password, done) => {
  if (address.length === 42) {
    address = address.toLowerCase(); // Use lower-case e-mails to avoid case-sensitive e-mail matching
  }
